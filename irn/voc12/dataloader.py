...
class PneumothoraxImageDataset(torch.utils.data.Dataset):
    def __init__(self, images_path_list, transform):
        self.images_df = pd.read_csv(images_path_list)
        self.images_dir = os.path.abspath(os.path.join(images_path_list, os.path.pardir))
    ...
    def __getitem__(self, idx):
        ...
        img = self.transform(img, mask=None).permute(2, 0, 1)

        return {'img': img, 'name': name}


class PneumothoraxAffinityDataset(torch.utils.data.Dataset):
    def __init__(self, images_path_list, transform, indices_from, indices_to):
        self.images_df = pd.read_csv(images_path_list)
        self.images_dir = os.path.abspath(os.path.join(images_path_list, os.path.pardir))
        self.transform = transform
        self.masks_path = os.path.join(self.images_dir, 'out_masks')
        self.extract_aff_lab_func = GetAffinityLabelFromIndices(indices_from, indices_to)
...
class PneumothoraxMSDataset(torch.utils.data.Dataset):
    def __init__(self, images_path_list, transform, scales=(1.0, 0.75, 0.5, 0.25)):
        self.images_df = pd.read_csv(images_path_list)
        self.images_dir = os.path.abspath(os.path.join(images_path_list, os.path.pardir))
        self.transform = transform
        self.scales = scales
    ...
    def __getitem__(self, idx):
        name, label = self.images_df.iloc[idx]
        label = torch.tensor([1.])  # always return label for first class
        ...
        # prepare different scales
        img = [F.interpolate(img[None], scale_factor=scale, mode='bicubic', align_corners=False)[0]
               for scale in self.scales]
        return {'img': img, 'name': name, 'label': label, 'size': size}


if __name__ == '__main__':
    from misc.indexing import PathIndex

    path_index = PathIndex(radius=10, default_size=(512 // 4, 12 // 4))

    train_dataset = VOC12AffinityDataset('train_aug.txt',
                                         label_dir='../result/ir_label',
                                         voc12_root='/datasets/LID/VOCdevkit/VOC2012',
    ...
    pack = train_dataset[25]
    img, label, aff_bg_pos_label, aff_fg_pos_label, aff_neg_label = pack['img'], pack['label'], \
                                                                    pack['aff_bg_pos_label'], pack['aff_fg_pos_label'], \
                                                                    pack['aff_neg_label']
    print(list(pack.keys()))
    print(type(img))
    print(type(label))
    print(np.unique(label))
    print(label.dtype)
    print(aff_bg_pos_label)
    print(aff_fg_pos_label)
    print(aff_neg_label)
